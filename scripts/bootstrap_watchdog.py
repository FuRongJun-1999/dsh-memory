...
# 生效条件：无入参；LINGSHU_PYTHON 环境变量为非空串且 os.path.isfile(env) 为真时返回该值，否则当 sys.executable 非空且其 basename 含 "python" 时返回 sys.executable，否则按 LOCALAPPDATA 拼出的 Programs\Python\Python3*\python.exe 与 %SystemDrive%\Python3*\python.exe 两个 glob 取 reverse 排序后首命中返回，两个模式均无命中时返回 cur or "python"（cur 为空串则回落 "python"）。
def resolve_python() -> str:
    ...
    for pat in (os.path.join(la, "Programs", "Python", "Python3*", "python.exe"),
                os.path.join(os.environ.get("SystemDrive", "C:") + os.sep, "Python3*", "python.exe")):
