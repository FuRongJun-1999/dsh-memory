...
```json
{
  ...
  "chars": 42,
  "observed_at": 1787183927.29, // **观测时间**（条件论「观测时间」栏，unix 秒）
  "time_window": [1787183927.29, 1787187527.29]  // 观测时间窗 [起, 止]
}
```

平均长度 136 字符。**观测时间覆盖率 1000/1000**，范围 **2026-08-14 ~ 2026-09-04**。

> **为什么必须带观测时间**：条件论中「观测时间」是条件空间的一栏——记忆的意义依赖
> 观测条件，脱离时间无法判断时效性、无法做时间衰减与按时间窗检索。缺了它，
> 「时序检索 / 遗忘曲线 / 新鲜度加权」这几类评测根本无从谈起。
>
...
| 本机项目路径 | **相对路径**（去掉盘符与本机父目录层，不暴露本机目录结构） |
| 其它绝对路径 | `<PATH>\最后一段` |
...
复检工具在私有仓库 `AEIS/tools/`（含敏感词表与私有数据，不随公开仓库分发），
生成物落在本地 `AEIS/data/public/`：

| 形态 | 生成器 | 用途 |
|---|---|---|
| **卡片页** | `make_review_page.py` | 快速浏览（按类别分组，带筛选/搜索）|
| **标注页** | `make_annotate_page.py` | 逐条标注（5 个动作、进度、本地持久化、导出修正 JSON）|
| **知识图谱页** | `make_graph_page.py` | **关联导航**：会话分簇 + 时序链，点节点看关联消息并标注 |

**知识图谱页要点**：
- 节点颜色 = 标签；**实心 = 已标注，空心 = 未标注**，红叉 = 判删除（一眼区分）
- 边 = 同会话时序链（还原对话线索）；虚线 = 源库显式边
- 点节点 → 右侧「关联消息」+「同会话其它消息」（带观测时间，可点击跳转）
- 支持深链 `#goto=<id>` 直接定位某条
- 标注与卡片页**共用 localStorage**，两边互通

**修正回流流程**：复检 → 导出 `memory-bench-1000-corrections.json` →
吸收为构建器 `EXCLUDE_ORIG_IDS` / 修正标签 → 重建 → 过门禁 `verify_bench.py`。
