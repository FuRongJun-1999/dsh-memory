...
**复检**：三道独立扫描全部 **0 命中**——
① `verify_bench_safety.py`（21 类隐私模式）
② `scan_bench_policy2.py`（性/成人 + 政治敏感）
③ `deep_scan.py`（凭据 KV + **熵检测** + 政策）
白名单仅放行自带 `PLACEHOLDER` 标识 / 哨兵值。
...
**复检**：`scan_bench_policy2.py` + `deep_scan.py` 独立重扫 → 性/成人 **0 命中**、政治敏感 **0 命中**。
...
---
*数据集由 `build_memory_bench.py` 生成；安全复检由 `verify_bench_safety.py` 执行（0 残留）。*
