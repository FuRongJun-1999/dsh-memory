//! hmac.rs · 纯 std SHA-256 + HMAC-SHA256（零 crate 依赖）
//! 对齐 Python hmac/hashlib 标准库语义（蜂群 instance_registry HMAC-SHA256 同源）。
//! FIPS 180-4 实现，确定性可审计——零依赖哲学下的手写密码学原语，
//! 用途仅限蜂群消息签名（B2 盲区修正同源），非通用密码学声明。
