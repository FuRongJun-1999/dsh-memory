...
**想做什么 → 找对应泳道 → 走条件边到功能**（流程图 = 认知图 = 条件路由图，**80 工具**全收录，[工具总表 → docs/灵枢MCP工具总表_v3.4.md](docs/灵枢MCP工具总表_v3.4.md)）：
...
- **三层关系**：知识真源（条件单元库）→ 说明书（技能包——何时用/怎么用/克制什么）→ 执行（**本插件挂载的灵枢 MCP 80 工具**·物理基底裁决）
...
- **白箱 wisdom_* 全工具**（`tools: all`）：80 个 MCP 工具含 wisdom_verify/analyze/predict/trust_judge/compose/respond/chat 白箱族，Agent 可直接调用。
...
灵枢 MCP server 注册 **80 个工具**，按心智功能分 11 大模块（智能论 v3.4：端口架构/锚定验证/认知图与条件路由/原生神经网络）。工具清单运行时动态拉取（灵枢升级 DSH 零改动），**完整总表（每个工具一条说明，不重复不遗漏）见 [docs/灵枢MCP工具总表_v3.4.md](docs/灵枢MCP工具总表_v3.4.md)**。下方为分类概览：
...
| 模式 | 暴露数 | 说明 |
|---|---|---|
| `'all'` | **67** | 80 个全量中排除 13 个**宿主级风险工具**（见下），含全部身体/视觉/白箱/角色/智慧之书能力 |
...
> 数字说明：MCP server 共注册 **80 个工具**（上面全量清单，运行时 tools/list 实测）；`'all'` 实际暴露 67 个（80-13 风险），`'brain'` 实际暴露 36 个，`'core'` 实际暴露 12 个。
