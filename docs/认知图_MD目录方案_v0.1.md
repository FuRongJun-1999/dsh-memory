...
在 3048 个真实节点上做分桶键普查（`md_cg/census.py`），四元组哈希**完全退化**：

| 方案 | 分桶键 | 桶数 | 最大桶占比 | 单例桶率 | 期望扫描占比 |
|---|---|---|---|---|---|
| A | 原始四元组哈希（v0.1） | **3037** | 0.1% | **99.9%** | **99.9%** |
| D | **归一化域键（v0.2 采用）** | **90** | 5.4% | **0.0%** | **2.0%** |

根因：`observation_position` 字段里嵌了**实例名**（如「知识点（二分查找）」），
共 3033 种取值 ≈ 节点数，一个节点一个桶 = 每次检索都退化成全表扫，
"条件路由非全量"的收益归零。另三个字段则**信息量过低**、无法承担分区：
`observation_tool` 93% 集中于单值、`existence_constraint` 95% 集中、`time_window` 仅 3 种取值。
...
| `{"tags": [...]}` 或 condition_space | 路由到对应桶，桶内优先 | 扫 166/3048 = 5.4% |
...
|---|---|---|---|
| 路由命中（T0） | — | — | **18.4x 加速**（中位） |
| 路由未命中（回退 T2） | — | — | 0.95x（白付一次桶扫描） |
| 无 context 全量（T2） | 183 ms | 161 ms | 1.1x |
| 写入吞吐 | **1887 节点/秒** | — | — |

> 命中与未命中必须**分开统计**——混在一起算中位数会把 18.4x 的加速稀释成看不出来的数字。
...
**校验必须全量，不能抽样**：~~抽样 10 节点字段等价~~ → 3048 节点**逐个**比对
`content/tags/importance/edges`，实测不一致 0 个。迁移是一次性不可逆动作，
抽样 10 个只能证明"没有系统性错误"，证明不了没有个别节点损坏
（例如正文含 `---` 分隔符会让朴素的 frontmatter 解析器丢光所有字段——
这个缺陷就是全量校验才暴露出来的，见 §2 注）。
...
### 代价/风险（v0.2 更新：括号内为实测状态）
1. **性能**：无 SQL 索引，靠内存 dict + 分区目录。**✅ 已 benchmark**：
   路由命中 18.4x 加速，无 context 全量 183ms vs sqlite 161ms（1.1x），写入 1887 节点/秒。
   前提是分桶键健康（§1.1），退化时性能收益归零 → 已做成常驻自检。
2. **一致性**：无 sqlite 事务。**✅ 已解决**：原子 rename + append-only 分片日志，
   9 轮 × 6 进程零丢失。**注意 v0.1 的"锁 + 读-改-写"方案实测是错的**（§6.1）。
3. **迁移成本**：一次性导出脚本 + 全量校验。**✅ 3048 节点字段等价，不一致 0**。**做之前先备份 sqlite**。
...
| P1 | 迁移脚本 + 全节点落盘 md | 节点数等价 + **全量**字段等价（非抽样） | **✅ 3048/3048，不一致 0** |
| P2 | 接入真实检索（替换 search_content 的 sqlite 分支）| **同查询集 recall@k 对比**，而非"结果一致" | **✅ 10 查询 avg recall@10 = 100%** |
| P3 | 并发锁 + benchmark + 回滚 | 多进程写零丢失 + 性能达标 | **✅ 9 轮×6 进程零丢失；18.4x/1.1x** |
...
- 风险1 分桶键：迁移等价、分区健康度自检、期望扫描 < 10%、**原方案能被自检判定为退化**
...
> **原则（大的修改要小心）**：P0 原型先证明"稳定+有效"，再逐阶段扩。**sqlite 保留做双写/回滚**，确认 md 稳定后才切。
...
---
*（v0.3：P0 25/25 + P1 37/37 全绿（`python -m md_cg.test_p0` + `python -m md_cg.test_p1`）。
