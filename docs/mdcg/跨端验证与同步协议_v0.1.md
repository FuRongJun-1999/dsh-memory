...
| **验证端** | zcode（在 D:/Program Files/4_ai/dsh-memory） | 独立复算与反证，产出**回执** | **无权设 task**；只出回执，不改台账 |
...
2. 验证端从主代理仓**直接 fetch**（本地路径即远端）：
   `git -C "D:/Program Files/4_ai/dsh-memory" fetch "D:/Program Files/2_ai/dsh-memory" main`
...
```
[VERIFY-REQ] verify_id=<verify_id> commit=<sha> repo=D:/Program Files/4_ai/dsh-memory
