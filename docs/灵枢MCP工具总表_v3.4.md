# 灵枢 AEIS MCP 工具总表（81 个 · 智能论 v3.4）
...
| `seven_layer_loop` | 七层闭环（里程碑3.4 · 阶段3收官：感知→记忆→理解→预测→验证→物理→决策完整自主循环，七层留痕可审计） |
| `world_generator` | 文字生图/文字生视频（里程碑4.3 · 世界模型生成器：文字→场景解析→世界实例化→3D 渲染；image 确定性 PNG / video GIF 多帧=世界演化+预测叠加） |
| `world_semantics` | 图像语义提取（里程碑4.5 · 感知方向：轮廓/形状/颜色/亮度→从外到内→图的信息定义；generate_roundtrip 生成-感知闭环自验证） |
...
**总计：81 个 MCP 工具 + 8 个引擎内部能力**（不重复、不遗漏）