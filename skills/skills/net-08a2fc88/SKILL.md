---
name: net-08a2fc88
description: >-
  路由收敛 / 网络-路由收敛 / 路由协议收敛——链路变化 / update 链。用户提到这些词时使用本技能。
  场景：对照：路由协议收敛——链路变化传播至全网稳定（收敛判定）。
  【不适用】Not for 以下场景：op 非 {converge, stable, update} 时
license: MIT
compatibility: >-
  op ∈ {converge, stable, update}
allowed-tools: Read Write Bash
metadata:
  version: "1.0"
  skill-author: 灵枢（AEIS）
  last-reviewed: "2026-08-29"
  kccs:
    trigger_words: ["路由收敛", "网络-路由收敛", "路由协议收敛——链路变化", "update 链"]
    when: "op ∈ {converge, stable, update}"
    sub: ["① op 分支处理"]
    execute: "按 op 分派"
    not_applicable: ["op 非 {converge, stable, update} 时"]
  calibration: "对照：路由协议收敛——链路变化传播至全网稳定（收敛判定）"
---

# 网络-路由收敛（net-08a2fc88）

## When to use

任务「路由收敛」；对照：路由协议收敛——链路变化传播至全网稳定（收敛判定）。

## 克制条款（不适用条件）

op 非 {converge, stable, update} 时

## How to execute

按 op 分派

## Verification

- 单元样例 4 条（cases 断言）
- 物理基底：按 calibration 对照（编译/运行/断言裁决）

## References

- 单元库：compiler_code_units.py「网络-路由收敛」
