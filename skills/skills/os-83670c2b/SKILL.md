...
name: os-83670c2b
description: >-
  文件压缩 / 文件-文件压缩 / compress RLE。用户提到这些词时使用本技能。
  场景：对照：文件压缩——RLE 行程编码（重复段压缩/还原）。
...
metadata:
  ...
  kccs:
    trigger_words: ["文件压缩", "文件-文件压缩", "compress RLE"]
...
任务「文件压缩」；对照：文件压缩——RLE 行程编码（重复段压缩/还原）。
...
- 单元库：compiler_code_units.py「文件-文件压缩」
