---
name: browser-22e8de36
description: >-
  触控手势 / 浏览器-触控手势 / tap 轻。用户提到这些词时使用本技能。
  场景：对照：Touch Events——轻点/平移/双指缩放手势识别。
  【不适用】Not for 以下场景：op 非 {pan, pinch, tap} 时
license: MIT
compatibility: >-
  op ∈ {pan, pinch, tap}
allowed-tools: Read Write Bash
metadata:
  version: "1.0"
  skill-author: 灵枢（AEIS）
  last-reviewed: "2026-08-29"
  kccs:
    trigger_words: ["触控手势", "浏览器-触控手势", "tap 轻"]
    when: "op ∈ {pan, pinch, tap}"
    sub: ["① op 分支处理"]
    execute: "按 op 分派；顺序调用"
    not_applicable: ["op 非 {pan, pinch, tap} 时"]
  calibration: "对照：Touch Events——轻点/平移/双指缩放手势识别"
---

# 浏览器-触控手势（browser-22e8de36）

## When to use

任务「触控手势」；对照：Touch Events——轻点/平移/双指缩放手势识别。

## 克制条款（不适用条件）

op 非 {pan, pinch, tap} 时

## How to execute

按 op 分派；顺序调用

## Verification

- 单元样例 3 条（cases 断言）
- 物理基底：按 calibration 对照（编译/运行/断言裁决）

## References

- 单元库：compiler_code_units.py「浏览器-触控手势」
