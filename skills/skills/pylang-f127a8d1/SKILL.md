---
name: pylang-f127a8d1
description: >-
  环境查询 / 工具-环境查询 / platform 平。用户提到这些词时使用本技能。
  场景：对照：sys/platform——平台与版本环境查询。
  【不适用】Not for 以下场景：op 非 {get, platform, version} 时
license: MIT
compatibility: >-
  op ∈ {get, platform, version}
allowed-tools: Read Write Bash
metadata:
  version: "1.0"
  skill-author: 灵枢（AEIS）
  last-reviewed: "2026-08-29"
  kccs:
    trigger_words: ["环境查询", "工具-环境查询", "platform 平"]
    when: "op ∈ {get, platform, version}"
    sub: ["① op 分支处理"]
    execute: "按 op 分派"
    not_applicable: ["op 非 {get, platform, version} 时"]
  calibration: "对照：sys/platform——平台与版本环境查询"
---

# 工具-环境查询（pylang-f127a8d1）

## When to use

任务「环境查询」；对照：sys/platform——平台与版本环境查询。

## 克制条款（不适用条件）

op 非 {get, platform, version} 时

## How to execute

按 op 分派

## Verification

- 单元样例 4 条（cases 断言）
- 物理基底：按 calibration 对照（编译/运行/断言裁决）

## References

- 单元库：compiler_code_units.py「工具-环境查询」
