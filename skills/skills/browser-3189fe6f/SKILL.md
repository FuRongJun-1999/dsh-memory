---
name: browser-3189fe6f
description: >-
  字体加载 / 浏览器-字体加载 / Font Loading / load 请。用户提到这些词时使用本技能。
  场景：对照：Font Loading API——字体加载状态与回退切换（font-display）。
  【不适用】Not for 以下场景：op 非 {load, status, swap} 时
license: MIT
compatibility: >-
  op ∈ {load, status, swap}
allowed-tools: Read Write Bash
metadata:
  version: "1.0"
  skill-author: 灵枢（AEIS）
  last-reviewed: "2026-08-29"
  kccs:
    trigger_words: ["字体加载", "浏览器-字体加载", "Font Loading", "load 请"]
    when: "op ∈ {load, status, swap}"
    sub: ["① op 分支处理"]
    execute: "按 op 分派"
    not_applicable: ["op 非 {load, status, swap} 时"]
  calibration: "对照：Font Loading API——字体加载状态与回退切换（font-display）"
---

# 浏览器-字体加载（browser-3189fe6f）

## When to use

任务「字体加载」；对照：Font Loading API——字体加载状态与回退切换（font-display）。

## 克制条款（不适用条件）

op 非 {load, status, swap} 时

## How to execute

按 op 分派

## Verification

- 单元样例 4 条（cases 断言）
- 物理基底：按 calibration 对照（编译/运行/断言裁决）

## References

- 单元库：compiler_code_units.py「浏览器-字体加载」
