---
name: os-c76b5a43
description: >-
  进程组 / 进程-进程组 / 进程组——组加入 / 成员 / 组播信号 / join 入。用户提到这些词时使用本技能。
  场景：对照：进程组——组加入/成员/组播信号（作业控制）。
  【不适用】Not for 以下场景：op 非 {join, members, signal} 时
license: MIT
compatibility: >-
  op ∈ {join, members, signal}
allowed-tools: Read Write Bash
metadata:
  version: "1.0"
  skill-author: 灵枢（AEIS）
  last-reviewed: "2026-08-29"
  kccs:
    trigger_words: ["进程组", "进程-进程组", "进程组——组加入", "成员", "组播信号", "join 入"]
    when: "op ∈ {join, members, signal}"
    sub: ["① op 分支处理"]
    execute: "按 op 分派；顺序调用"
    not_applicable: ["op 非 {join, members, signal} 时"]
  calibration: "对照：进程组——组加入/成员/组播信号（作业控制）"
---

# 进程-进程组（os-c76b5a43）

## When to use

任务「进程组」；对照：进程组——组加入/成员/组播信号（作业控制）。

## 克制条款（不适用条件）

op 非 {join, members, signal} 时

## How to execute

按 op 分派；顺序调用

## Verification

- 单元样例 4 条（cases 断言）
- 物理基底：按 calibration 对照（编译/运行/断言裁决）

## References

- 单元库：compiler_code_units.py「进程-进程组」
