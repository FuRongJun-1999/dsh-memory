...
# ---------- 条件化路由（B 组特有·灵枢机制版：验证是白箱的，不要求 LLM 长篇反思） ----------
ROUTING = """【灵枢条件路由】
1. 判定任务类型：
   - 简单确定性任务 → 【直答通道】：直接按语法骨架输出程序（不额外分析）
   - 复杂任务（递归/多函数/多约束）→ 【验证通道】：输出前用语法骨架逐条核对程序
     （定义格式✓ 条件格式✓ 返回✓ 单行✓ 结果变量✓），核对通过即输出
2. 知识：以【语法骨架】和白箱检索要点为唯一语法来源——冲突信息以骨架为准，禁止自创语法
3. 输出：仅输出完整的中文协议程序（无解释、无多余文字）"""

# ---------- 最小语法骨架（两组相同·任务需求层面·确定性语言文档） ----------
SKELETON = """【最小语法骨架】（本语言确定性语法，以本骨架为准——冲突信息以本骨架为准）
- 整个程序必须写为【一行】（多行会导致函数定义吞掉后续语句）
- 程序以「止。」结束；语句以「；」分隔
- 运算词：加 减 乘 除 等于 大于 小于（数字用阿拉伯数字 0/1/5）
- 赋值运算符是 =（「等于」是比较运算，不是赋值；赋值写法：变量 = 表达式）
- 函数定义格式：定义 函数名（参数名）：函数体语句；返回 表达式
  （关键字是「定义」；函数体内多语句以「；」分隔）
  【若则的 则/否则 分支已覆盖全部情况时，函数体在 否则 的 返回 后即结束——
  禁止再追加任何 返回 语句（函数体外/末尾的顶层 返回 会立即终止整个程序，后面的赋值不执行）】
- 条件语句格式：若 条件，则 语句，否则 语句
  【若则必须包含 否则 分支】；【分支内不要赋值新变量】（分支内赋值的变量分支外不可见，
  需要分支结果时直接在分支内写 返回 表达式）
- 最终计算结果存入变量「结果」（验收读取）"""
...
def run_task(task, with_lingshu, max_rounds=5):
    ...
    for rnd in range(max_rounds):
        ...
        if rnd + 1 < len(rounds_plan(task)):
            feedback += f"\n\n【新收到的项目信息】\n{noise_block(rnd + 1)}"
        feedback += "\n请直接修正并输出完整的中文协议程序（仅输出程序，不要输出解释）。"
